...
// trebuie pastrata ordinea: int rows, int cols,double matrix[rows][cols]
...
void printArray(int col, double arr[col]){
    int i; 
    for (i = 0; i < col; i++) {
        printf("b[%d] = %10.4lf \n", i, arr[i]);
    }
...
void pivotare(int n, double a[n][n], double b[n]){
    int k,i,j; 
    double p; 
    for (k = 0; k < n-1; k++) {
        for (i = k + 1; i < n; i++) {
...
int main() {
    ...
    printf("Dupa pivotare:\n");
    printMatrix(n, n, a);

    printf("Vectorul b dupa pivotare:\n");
    printArray(n, b);
